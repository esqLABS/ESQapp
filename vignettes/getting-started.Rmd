...
In this example, we will use an examplary project created by executing
...
![](Figures/getting-started/fig1.png)

Click on the button "Select Project Configuration" and select the
...
![](Figures/getting-started/fig2.png)
...
or add a new one, right click on a row and select either "Insert" or
"Remove row".

![](Figures/getting-started/fig3.png)

The main properties that can be defined for a scenario are:

-   Individual is specified for a scenario in the column `IndividualId`,
    which shows a drop-down-list with all individuals defined in the
    `Individuals` tab. To unassign an individual from a scenario, right
    click on the cell and select "Clear".

-   Population is specified for a scenario in the column `PopulationId`,
    which shows a drop-down-list with all populations defined in the
    `Populations`tab. To unassign an population from a scenario, right
    click on the cell and select "Clear".

-   To edit the **simulation time** of the scenario, right click on the
    `SimulationTime` entry and select "Enter Simulation Time".\
    ![](Figures/getting-started/fig4.png)
...
`Individuals`-tab. Currently, it is only possible to add new individual
biometrics, but not to add new individual parametrization sheets. This
step must be performed in the `Individuals.xlsx` excel file as
[descirbed in the
documentation](https://esqlabs.github.io/esqlabsR/articles/esqlabsR-design-scenarios.html#individuals)
of `` {esqlabsR}` ``. Furthermore, definition of protein ontogenies are
...
![](Figures/getting-started/fig6.png)

In the current version of the app, it is not possible to add new or to
remove the existing sets. This must be performed in the excel file.
...
![](Figures/getting-started/fig7.png)
...
![](Figures/getting-started/fig8.png)

The changes will be written into the excel files. Of course, manually
editing the excel files still remains the option.
