...
#### **Main Changes:**
- **Scenarios -> ModelFile column. Added support for manual text entry and dropdown selection from available file options.** Resolved an issue in (#180).
- **Handsontable Table rendering: Disabled full table rerendering when individual cell content is edited or changed.** Resolved an issue in (#180).
